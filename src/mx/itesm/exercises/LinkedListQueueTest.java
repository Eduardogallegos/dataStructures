...
class LinkedListQueueTest {
	...
	private static Collection<Integer> integerCollection = Arrays.asList(4, 8, 15, 16, 23, 42);
	private static Collection<String> stringCollection = Arrays.asList("Fili", "Kili", "Oin", "Gloin", "Thorin",
			"Dwalin", "Balin", "Bifur", "Bofur", "Bombur", "Dori", "Nori", "Ori");
	private static Collection<Double> doubleCollection = Arrays.asList(2.99792458e8, 6.67408e-11, 6.62607015e-34,
			1.602176634e-19, 6.02214076e23);

	@BeforeEach
	void setUp() throws Exception {
		emptyQueue = new LinkedListQueue<>();
		integerQueue = new LinkedListQueue<>(integerCollection);
		stringQueue = new LinkedListQueue<>(stringCollection);
		doubleQueue = new LinkedListQueue<>(doubleCollection);
	}
	...
	void testOffer() {
		...
		assertThrows(NullPointerException.class, () -> integerQueue.offer(null));
		...
		assertThrows(NullPointerException.class, () -> stringQueue.offer(null));
		assertEquals(16, stringQueue.size());

		assertTrue(doubleQueue.offer(101325.0));
		assertThrows(NullPointerException.class, () -> doubleQueue.offer(null));
	...
	void testPoll() {
		...
		assertEquals((Double) 2.99792458e8, doubleQueue.poll());
	...
	void testPeek() {
		...
		assertEquals((Double) 2.99792458e8, doubleQueue.peek());
		assertEquals(5, doubleQueue.size());
		assertEquals((Double) 2.99792458e8, doubleQueue.peek());
	}
	...
	@Test
	void testIterator() {
		Iterator<Object> emptyIterator = emptyQueue.iterator();
		assertFalse(emptyIterator.hasNext());
		assertThrows(NoSuchElementException.class, () -> emptyIterator.next());

		Iterator<String> stringIterator1 = stringQueue.iterator();
		...
		while (stringIterator1.hasNext()) {
			...
			assertEquals(stringIterator1.next(), stringIterator2.next());
		}
		assertFalse(stringIterator1.hasNext());
		assertFalse(stringIterator2.hasNext());
		assertThrows(NoSuchElementException.class, () -> stringIterator1.next());
		assertThrows(NoSuchElementException.class, () -> stringIterator2.next());

		assertIterableEquals(integerCollection, integerQueue);
		...
		assertEquals((Integer) 4, integerQueue.poll());
		assertTrue(integerQueue.offer(108));
		assertEquals("[8, 15, 16, 23, 42, 108]", integerQueue.toString());
		...
		assertTrue(stringQueue.offer("Beorn"));
		assertEquals("Oin", stringQueue.poll());
		assertEquals("[Gloin, Thorin, Dwalin, Balin, " + "Bifur, Bofur, Bombur, " + "Dori, Nori, Ori, Frodo, "
				+ "Gandalf, Beorn]", stringQueue.toString());

		assertEquals("[2.99792458E8, 6.67408E-11, 6.62607015E-34, " + "1.602176634E-19, 6.02214076E23]",
